...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .frame(width: 280, height: calculatedHeight)
    ...
    private var calculatedHeight: CGFloat {
        ...
        if appItems.isEmpty {
            return headerHeight + footerHeight + 60
        }
        let rowHeight: CGFloat = 44
        let sectionHeaderHeight: CGFloat = 28
        let listHeight = CGFloat(appItems.count) * rowHeight + sectionHeaderHeight
        return min(headerHeight + footerHeight + listHeight, 480)
    }
    ...
    private var footerView: some View {
        ...
        .background(
            VisualEffectBlur(material: .menu, blendingMode: .withinWindow)
                .overlay(Rectangle().frame(height: 1).foregroundColor(Color.primary.opacity(0.05)), alignment: .top)
        )
    }
}
